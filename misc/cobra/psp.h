...
{
	system_call_4(11, SYSCALL8_OPCODE_SET_PSP_UMDFILE, (uint64_t)(uint32_t)file, (uint64_t)(uint32_t)id, prometheus);
	return (int)p1;
}
...
{
	system_call_8(11, SYSCALL8_OPCODE_SET_PSP_DECRYPT_OPTIONS, decrypt_patch, tag, (uint64_t)(uint32_t)keys, code, tag2, (uint64_t)(uint32_t)keys2, code2);
	return (int)p1;
}

static inline int sys_psp_change_emu_path(const char *path)
{
	system_call_2(11, SYSCALL8_OPCODE_PSP_CHANGE_EMU, (uint64_t)(uint32_t)path);
	return (int)p1;
}
