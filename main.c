...
#endif
#include "slaunch/include/mem.h"
#include "slaunch/include/misc.h"
#endif
#endif
...
static char paths [10][16]={"GAMES", "GAMEZ", "PS3ISO", "BDISO", "DVDISO", "PS2ISO", "PSXISO", "PSXGAMES", "PSPISO", "ISO"};
...
/* #define BEEP1 { system_call_3(392, 0x1004, 0x4,   0x6); }
#define BEEP2 { system_call_3(392, 0x1004, 0x7,  0x36); }
#define BEEP3 { system_call_3(392, 0x1004, 0xA, 0x1B6); }
#define BEEPX { system_call_3(392, 0x1004, 0xA, 0x0FFF); } */
...
#define BEEPX { system_call_3(392, 0x1007, 0xA, 0x01E6); } //long BEEP2
#define RANGE(a, b, c)	((a) <= (b) ? (b) : (a) >= (c) ? (c) : (a))
// #define MYSPRX	"/dev_hdd0/tmp/MySPRX.sprx"
// #define CONFIG_HDD	"/dev_hdd0/sprx.txt"
// #define MAX_GAME_PLUGINS		3
// #define GAME_PLUGINS_FIRST_SLOT		1
// #define MAX_EBOOT_PLUGINS 		(MAX_GAME_PLUGINS-GAME_PLUGINS_FIRST_SLOT)
#define process_id_t uint32_t
#define sys_prx_id_t int32_t
#define uint	unsigned int
#define scanInc 0x1000
struct CellPadData		pdata;
#ifdef ART
sys_ppu_thread_t thread_id_art		=-1;
const char memBuf[scanInc];
const int second = 1000000;
process_id_t attachedPID = 0;
//Config
int isDEX = 0, doForceWrite = 0, isConstantWrite = 0, exitThread = 0, check_turnoff_delay = 0, check_turnoff_delay_max = 10;
#ifdef DISABLER
const int isDisabler1 = 0, isDisabler2 = 0, isDisabler3 = 0, isDisabler4 = 0, isDisabler5 = 0, isDisabler6 = 0, isDisabler7 = 0, isDisabler8 = 0;
#endif
#ifdef CODETYPE
char lineBuf[100], * userCodes = NULL, * typeA_Copy = NULL;
uint typeA_Size = 0;
#else
char lineBuf[100], * userCodes = NULL;
#endif
uint64_t ptrAddr = 0;
...
int get_process_mem(process_id_t pid, uint64_t addr, char *buf, int size, int isDEX);
int set_process_mem(process_id_t pid, uint64_t addr, char * buf, int size, int isDEX);
...
void art_process(int forceWrite);
int check_turnoff(void);
void check_syscall_api(void);
void art_thread(uint64_t arg);
#endif
u32 BUFFER_SIZE_PS3MAPI = (65536UL);
sys_prx_id_t load_start(const char *path);
sys_prx_id_t stop_unload(sys_prx_id_t id);
void get_value(char *text, char *url, u16 size);
int val(const char *c);
void add_option_item(const int conn_s, const char *value, const char *label, bool selected, char *buffer);
uint64_t convertH(char *val);
int Char2Int(char input);
void Hex2Bin(const char* src, char* target);
void ps3mapi_vshplugin(const int conn_s, char *buffer, char *templn, char *param);
void ps3mapi_gameplugin(const int conn_s, char *buffer, char *templn, char *param);
void ps3mapi_home(const int conn_s, char *buffer, char *templn);
// int check_for_booted_game(void); //credits to OsirisX (vshmenu)
int sm_stop(void);
int sm_exit(void);
#endif
...
static void prepNTFS(int conn_s)
{
	...
	char c_path[5][9]={"PS3ISO", "BDISO", "DVDISO", "PSXISO", "PSXGAMES"};
	char prefix[2][8]={"/", "/PS3/"};
...
/* 	//add plugins list
	{
		...
		int fd, cnt = 0; char paths[6][32] = {"/dev_hdd0/z/plugins/vsh", "/dev_hdd0/z/plugins/game", "/dev_hdd0/z/plugins/recording", "/dev_hdd0/tmp"};
		char paths[6][32] = {"/dev_hdd0/z/plugins/vsh"};

		for(u8 i = 0; i < 6; i++)
		if(cellFsOpendir(paths[i], &fd) == CELL_FS_SUCCEEDED)
		// if(cellFsOpendir(path1[i], &fd) == CELL_FS_SUCCEEDED)
		{
	...
	if(!is_ps3mapi_home) ssend(conn_s, "<hr color=\"#FF0000\"/>");
	ssend(conn_s, templn);
	ssend(conn_s, "</td></tr></table><hr color=\"#0099FF\"/>");
}
		...
		else
		{
			...
			if(pos)
			{
				...
				if(strstr(prx_path, "dev_flash"))
					...
					{system_call_6(8, SYSCALL8_OPCODE_PS3MAPI, PS3MAPI_OPCODE_LOAD_PROC_MODULE, (u64)pid, (u64)(u32)prx_path, NULL, 0);}
			}
		}
	}
loadgameplug_err_arg:
	if(!is_ps3mapi_home)
		sprintf(templn, "<b>%s --> %s</b>"
						"<hr color=\"#0099FF\"/>",
						"PS3MAPI", "Process Plugins");
	else
		sprintf(templn, "<b>%s</b><hr color=\"#0099FF\"/>", "Process Plugins");
	ssend(conn_s, templn);
	sprintf(templn, "<form action=\"/gameplugin.ps3mapi\" method=\"get\" enctype=\"application/x-www-form-urlencoded\" target=\"_self\"><br>"
					"<b><u>%s: </u></b>  ", "Process");
	ssend(conn_s, templn); memset(templn, 0, 512);
	if(pid == 0)
	{
		ssend(conn_s, "<select name=\"proc\">");
		char pid_str[32];
		u32 pid_list[16];
		...
		for(int i = 0; i < 16; i++)
		{
			if(1 < pid_list[i])
			{
				...
				if(1 < strlen(templn))add_option_item(conn_s, pid_str , templn, true, buffer);
			}
		}
		ssend(conn_s, "</select>   ");
	}
	...
	if(pid != 0)
	{
		...
		{system_call_4(8, SYSCALL8_OPCODE_PS3MAPI, PS3MAPI_OPCODE_GET_ALL_PROC_MODULE_PID, (u64)pid, (u64)(u32)mod_list);}
		for(unsigned int slot = 0; slot < 56; slot++)
		{
			...
			if(1 < mod_list[slot])
			{
				{system_call_5(8, SYSCALL8_OPCODE_PS3MAPI, PS3MAPI_OPCODE_GET_PROC_MODULE_NAME, (u64)pid, (u64)mod_list[slot], (u64)(u32)tmp_name);}
				{system_call_5(8, SYSCALL8_OPCODE_PS3MAPI, PS3MAPI_OPCODE_GET_PROC_MODULE_FILENAME, (u64)pid, (u64)mod_list[slot], (u64)(u32)tmp_filename);}
			}
			if(strlen(tmp_filename) > 0)
			{
				sprintf(templn,
	...
	sprintf(templn, "%s", "</table><br>");
	if(!is_ps3mapi_home) ssend(conn_s, "<hr color=\"#0099FF\"/>");
	ssend(conn_s, templn);
}
...
/* #ifndef MYSTUFF
	...
	// if (syscall8_state>=0 && syscall8_state<3 )
	{
		...
		//VSH Plugin-----------------------------------
		//---------------------------------------------
		ps3mapi_vshplugin(conn_s, buffer, templn, (char*)" ");

		//---------------------------------------------
		//Game Plugin-----------------------------------
		//---------------------------------------------
		ps3mapi_gameplugin(conn_s, buffer, templn, (char*)" ");
	}
#ifdef FULL
	//VSHPlugin
	ps3mapi_vshplugin(conn_s, buffer, templn, (char*)" ");
...
/* int check_for_booted_game(void)
{
	int32_t r;
	if (FindLoadedPlugin("game_plugin") != 0) {
		...
		int len;
		char tmp[256];
		int current_slot = GAME_PLUGINS_FIRST_SLOT;
		int num_loaded = 0;
		...
		{
			char gamesprx[128];
			strcpy(gamesprx, MYSPRX);
			// r = load_plugin_to_game((uint64_t)MYSPRX, 0, 0);
			r = cobra_load_game_plugin(current_slot, gamesprx, 0, 0);
			if (r == 0)
			{
		...
		while (num_loaded < MAX_EBOOT_PLUGINS)
		{
			...
			if(fgets(line, sizeof line, f) != NULL)
			{
				...
				r = cobra_load_game_plugin(current_slot, line, 0, 0);
				if (r >= 0)
				{
					sprintf((char*)tmp, "PRX geladen\nPFAD: %s", line);
					vshtask_notify(tmp);
...
int WriteMem(unsigned long pid, uint64_t addr, char * buf, int size)
{
	...
	if (size > 4 && 0) //Can't seem to write too many bytes at once
	{
		for (int z = 0; z < size; z += 4)
		{
			...
			if (addr != 0)
			{
				ret1 = set_process_mem(pid, addr + z, bufTemp, zl, isDEX);
				//if (ret1 != 0)
	...
	else
	{
		if (addr != 0)
		{
			ret1 = set_process_mem(pid, addr, buf, size, isDEX);
			//if (ret1 != 0)
...
{
	char cmp[bufLen];
	if (get_process_mem(pid, addr, cmp, bufLen, isDEX) < 0)
		return 0;
	...
	if (skip[0] <= 0)
	{
		...
		switch (cType)
		{
...
#else
				switch (arg0) //Write bytes (1=OR,2=AND,3=XOR,rest=write)
				{
					case 1: //OR
						ReadHex(lines, (start + lineLen) - arg2Len + 1, arg2Len - 1, buf0, 4);
						//Get source bytes
						get_process_mem(pid, addr, buf1, (arg2Len/2), isDEX, isCCAPI);
						// get_process_mem(pid, addr, buf2, (arg2Len/2), isDEX, isCCAPI);
						//Or each byte
			...
			case '4': ; //Write condensed
				//Get count
				uint count = (uint)(((unsigned char)buf1_2[0] << 24) | 
									((unsigned char)buf1_2[1] << 16) | 
									((unsigned char)buf1_2[2] << 8) | 
									((unsigned char)buf1_2[3]));
				//Get increment
				uint64_t inc = (uint64_t)(((unsigned char)buf0_2[0] << 24) | 
										((  unsigned char)buf0_2[1] << 16) | 
										((  unsigned char)buf0_2[2] << 8) | 
										((  unsigned char)buf0_2[3]));
				//Get write
 			...
 			case '5': //Disabler
				if (isDisabler)
				{
					...
					ReadHex(lines, (start + lineLen) - arg2Len + 1, arg2Len - 1, buf0, 4);
					//Get source bytes
					get_process_mem(pid, addr, buf0, (arg2Len/2), isDEX);
					// Write bytes to dest
					WriteMem(pid, addr, buf0, (arg2Len/2));
				}
				break;
			...
			case '6': //Write pointer
				...
				uint64_t offset = (uint64_t)(((unsigned char)buf0[0] << 24) | 
											(( unsigned char)buf0[1] << 16) | 
											(( unsigned char)buf0[2] << 8) | 
											(( unsigned char)buf0[3]));
				//Get address at pointer
				get_process_mem(pid, addr, buf0, 4, isDEX);
				ptrAddr = (uint64_t)(((unsigned char)buf0[0] << 24) | 
									(( unsigned char)buf0[1] << 16) | 
									(( unsigned char)buf0[2] << 8) | 
									(( unsigned char)buf0[3]));
				ptrAddr += offset;
			...
			case 'A': //Copy paste
				switch (arg0)
				{
					case 1:
						...
						//Get count
						ReadHexPartial(lines, (start + lineLen) - arg2Len + 1, arg2Len - 1, buf0, (arg2Len - 1)/2);
						uint count = (uint)(((unsigned char)buf0[0] << 24) | 
											((unsigned char)buf0[1] << 16) | 
											((unsigned char)buf0[2] << 8) | 
											((unsigned char)buf0[3]));
						typeA_Size = count;
						...
						get_process_mem(pid, addr, typeA_Copy, 4, isDEX);
...
#endif
			case 'B': //Find Replace
				//Only work when doForceWrite is true (1) 
				//Don't want constantly searching
				if (!doForceWrite)
					break;
				//Get end addr
				ReadHex(lines, (start + lineLen) - arg2Len + 1, arg2Len - 1, addrBuf, 4);
				uint64_t endAddr = (uint64_t)(((unsigned char)addrBuf[0] << 24) | 
											((  unsigned char)addrBuf[1] << 16) | 
											((  unsigned char)addrBuf[2] << 8) | 
				...
				{
					if (get_process_mem(pid, curAddr, (char *)memBuf, scanInc, isDEX) >= 0)
					{
						//So we stop it each loop before (scanInc - cmpSize) 
						//that the result is the last 2 bytes, for instance, 
						//and the compare is actually 4 bytes 
						//(so it won't result even though it should)
						//This fixes that
						for (uint64_t boff = 0; boff < (scanInc - cmpSize); boff++)
						{
							...
							if (CompareMemoryBuffered((char *)memBuf, boff, buf0_2, cmpSize))
							{
								...
								//Just skip in case the replacement has the ogp
								//We subtract one because it gets added back on top of the loop
								boff += binc - 1;
								temp++;
							}
						}
			...
			case 'F': //Copy bytes
				//Get destination
				ReadHex(lines, (start + lineLen) - arg2Len + 1, arg2Len - 1, buf0, 4);
				uint64_t dest = (uint64_t)((( unsigned char)buf0[0] << 24) | 
											((unsigned char)buf0[1] << 16) | 
											((unsigned char)buf0[2] << 8) | 
											((unsigned char)buf0[3]));
				//Get source bytes
				get_process_mem(pid, addr, buf1, arg0, isDEX);
				//Write bytes to dest
...
/* 							if (isConstantWrite || doForceWrite)
							{
								if (isDisabler1)
								{
									x = ParseLine2(pid, ncCodes, x, totalLen, skip, doForceWrite);
								}
								else
								if (isDisabler2)
								{
									x = ParseLine3(pid, ncCodes, x, totalLen, skip, doForceWrite);
								}
								else
								if (isDisabler3)
								{
									x = ParseLine4(pid, ncCodes, x, totalLen, skip, doForceWrite);
								}
								else
								if (isDisabler4)
								{
									x = ParseLine5(pid, ncCodes, x, totalLen, skip, doForceWrite);
								}
								else
								if (isDisabler5)
								{
									x = ParseLine6(pid, ncCodes, x, totalLen, skip, doForceWrite);
								}
								else
								if (isDisabler6)
								{
									x = ParseLine7(pid, ncCodes, x, totalLen, skip, doForceWrite);
								}
								else
								if (isDisabler7)
								{
									x = ParseLine8(pid, ncCodes, x, totalLen, skip, doForceWrite);
								}
								else
								if (isDisabler8)
								{
									x = ParseLine9(pid, ncCodes, x, totalLen, skip, doForceWrite);
								}
								else
									x = ParseLine(pid, ncCodes, x, totalLen, skip, doForceWrite);
							}
...
{
	if(isDEX)
		// printf("ArtemisPS3 : Using DEX Syscalls\n");
		vshtask_notify("ArtemisPS3 : Using DEX Syscalls");
	else
		// printf("ArtemisPS3 : Using COBRA Syscalls\n");
		vshtask_notify("ArtemisPS3 : Using COBRA Syscalls");
}
...
void art_thread(uint64_t arg)
{
...
#ifdef DEBUG
	printf("ArtemisPS3 : Thread Started\n");
#endif
	vshtask_notify("ArtemisPS3 : Ready");
	...
	while (1)
	{
		...
		if (GameProcessID != 0)
		{
			...
			{
#ifdef DEBUG
				printf("ArtemisPS3 : Process Exit\n");
#endif
				destroy_heap();
				attachedPID = 0;
			}
			else
			{
				if (GameProcessID != lastGameProcessID)
				{
					...
					for (int x = 0; x < (4 * 100); x++) //ingame 4 second delay (do not interfere with trophy/save stuff)
				...
				if(pdata.len > 0)
				{
					curpad = (pdata.button[2] | (pdata.button[3] << 8));
 					if (curpad != oldpad)
					{
						if(((curpad & PAD_L3) && (curpad & PAD_R3)) && (!(curpad & PAD_START) && !(curpad & PAD_SELECT)))
						{
							attachedPID = GameProcessID;
							if (attachedPID)
							{
#ifdef DEBUG
								printf("ArtemisPS3 : Attached to 0x%08X\n", attachedPID);
#endif
								destroy_heap();
								if (get_heap() == 0)
									create_heap(2, 1);
								...
								art_process(1);
								play_rco_sound("snd_trophy");
								while ((cellPadGetData(0, &pdata) | 1) && pdata.len > 0)
								{
									if (!((curpad & PAD_L3) && (curpad & PAD_R3)))
										break;
...
#ifdef DISABLER
						else if(((curpad & PAD_L3) && !(curpad & PAD_R3)) || (!(curpad & PAD_START) && !(curpad & PAD_SELECT)))
						{
							if(curpad & PAD_UP)
							{
								attachedPID = GameProcessID;
								if (attachedPID)
								{
									...
									while ((cellPadGetData(0, &pdata) | 1) && pdata.len > 0)
									{
										if (!((curpad & PAD_L3) && (curpad & PAD_UP)))
											break;
...
/* 							if(curpad & PAD_RIGHT)
							{
								...
								if (attachedPID)
								{
									...
									while ((cellPadGetData(0, &pdata) | 1) && pdata.len > 0)
									{
										if (!((curpad & PAD_L3) && (curpad & PAD_RIGHT)))
											break;
							...
							if(curpad & PAD_DOWN)
							{
								...
								if (attachedPID)
								{
									...
									while ((cellPadGetData(0, &pdata) | 1) && pdata.len > 0)
									{
										if (!((curpad & PAD_L3) && (curpad & PAD_DOWN)))
											break;
							...
							if(curpad & PAD_LEFT)
							{
								...
								if (attachedPID)
								{
									...
									while ((cellPadGetData(0, &pdata) | 1) && pdata.len > 0)
									{
										if (!((curpad & PAD_L3) && (curpad & PAD_LEFT)))
											break;
...
/* 						else if (((curpad & PAD_R3) && !(curpad & PAD_L3)) || (!(curpad & PAD_START) && !(curpad & PAD_SELECT)))
						{
							if(curpad & PAD_UP)
							{
								attachedPID = GameProcessID;
								if (attachedPID)
								{
									...
									while ((cellPadGetData(0, &pdata) | 1) && pdata.len > 0)
									{
										if (!((curpad & PAD_R3) && (curpad & PAD_UP)))
											break;
							...
							if(curpad & PAD_RIGHT)
							{
								...
								if (attachedPID)
								{
									...
									while ((cellPadGetData(0, &pdata) | 1) && pdata.len > 0)
									{
										if (!((curpad & PAD_R3) && (curpad & PAD_RIGHT)))
											break;
							...
							if(curpad & PAD_DOWN)
							{
								...
								if (attachedPID)
								{
									...
									while ((cellPadGetData(0, &pdata) | 1) && pdata.len > 0)
									{
										if (!((curpad & PAD_R3) && (curpad & PAD_DOWN)))
											break;
							...
							if(curpad & PAD_LEFT)
							{
								...
								if (attachedPID)
								{
									...
									while ((cellPadGetData(0, &pdata) | 1) && pdata.len > 0)
									{
										if (!((curpad & PAD_R3) && (curpad & PAD_LEFT)))
											break;
...
#endif
						else if (((curpad & PAD_START) && (curpad & PAD_SELECT)) && (!(curpad & PAD_L3) && !(curpad & PAD_R3)))
						{
#ifdef DEBUG
							printf("ArtemisPS3 : Bye Bye\n");
#endif
							vshtask_notify("ArtemisPS3 : Bye Bye");
							attachedPID = 0;
							goto exit;
						}
						// else
				...
				{
#ifdef DISABLER
					if (isDisabler1 || isDisabler2 || isDisabler3 || isDisabler4 || isDisabler5 || isDisabler6 || isDisabler7 || isDisabler8)
						art_process(0);
					else
#endif
						art_process(0);
...
static void content_scan(u64 arg0)
{
		...
		char* addr		=(char*)malloc(8192);	if(!addr) {init_running=0; return;}
...
leave_file:
		...
		else
		{
			...
			{
				sysmem=0;
				if(sys_memory_allocate(BUFFER_SIZE_HTTP2, SYS_MEMORY_PAGE_SIZE_64K, &sysmem)!=0)
					goto leave2;
				char *buffer= (char*)sysmem;
				if(strstr(param,"home.ps3mapi"))
				{
					ps3mapi_home(conn_s, buffer, templn);
				}
				else
				if(strstr(param, "vshplugin.ps3mapi"))
				{
					ps3mapi_vshplugin(conn_s, buffer, templn, param);
				}
				else
				if(strstr(param,"gameplugin.ps3mapi"))
				{
					ps3mapi_gameplugin(conn_s, buffer, templn, param);
				}
			}
#endif
			else
...
static void poll_thread(u64 poll)
{
	...
	while(working)
	{
/* 		if(max_temp && !(to&3)) //get freeze on my CECHL
		{
			t1=0;
			get_temperature(0, &t1); // 3E030000 -> 3E.03'C -> 62.(03/256)'C
			t2=t1;
		...
		if(max_temp)
		{
			...
			else
			{
				...
				if(delta> 0)
				{
					...
					{
						fan_speed--;
						if(t1 <= (max_temp - 3)) {fan_speed--;} // decrease fan speed faster if > 66% & cpu is cool
						if(t1 <= (max_temp - 5)) {fan_speed--;} // decrease fan speed faster if > 50% & cpu is very cool
						smoothstep=0;
					}
				}
			}
			...
			// if(t1>76 && old_fan<0x43) fan_speed++;
			if(t1>76 && old_fan<0x66) fan_speed+=step_up;
			if(t1>84 && fan_speed<0xB0) {old_fan=0; fan_speed=0xB0;}
			...
			if(old_fan!=fan_speed || stall>35)
			{
				//if(t1>76 && fan_speed<0x50) fan_speed=0x50;
				//if(t1>77 && fan_speed<0x58) fan_speed=0x58;
				if(t1>70 && fan_speed<0x50) fan_speed+=2;
				if(old_fan!=fan_speed)
				{
					old_fan=fan_speed;
					fan_control(fan_speed, 1);
		...
		if(pdata.len > 0) {
			curpad = (pdata.button[2] | (pdata.button[3] << 8));
			if((curpad & PAD_R1)){
				if((curpad & PAD_START) && !(curpad & PAD_CIRCLE)){
					int ret, ppu_prio;
					//pseudo check if Artemis is already active
					ret = sys_ppu_thread_get_priority(thread_id_art, &ppu_prio);
					if (ret != CELL_OK) {
						sys_ppu_thread_create(&thread_id_art, art_thread, 0, 3071, 0x200, 0, "artt"); //highest priority for Artemis
						BEEP1
					}
				}
/* 				else if((curpad & PAD_CIRCLE) && !(curpad & PAD_START)) {
...
*/
		...
		{
			pdata.len=0;
			if (infobuf.port_status[0] == CELL_PAD_STATUS_CONNECTED && (cellPadGetData(0, &pdata) == CELL_PAD_OK) && pdata.len) goto pad_ok;
			if (infobuf.port_status[1] == CELL_PAD_STATUS_CONNECTED && (cellPadGetData(1, &pdata) == CELL_PAD_OK) && pdata.len) goto pad_ok;
		}
		else
			sys_timer_sleep(2);
...
pad_ok:
		if ((pdata.button[CELL_PAD_BTN_OFFSET_DIGITAL2] & CELL_PAD_CTRL_L2) && (pdata.button[CELL_PAD_BTN_OFFSET_DIGITAL2] & CELL_PAD_CTRL_R2))
		{
			if(gui_allowed(1))
			{
				sys_ppu_thread_create(&thread_id_gui, slaunch_thread, 0, 2000, 0x2000, SYS_PPU_THREAD_CREATE_JOINABLE, THREAD_MENU);
		...
		if ((pdata.button[CELL_PAD_BTN_OFFSET_DIGITAL1] & CELL_PAD_CTRL_SELECT))
		{
			if( !(sm_config->combo & FAIL_SAFE)
				&& (pdata.button[CELL_PAD_BTN_OFFSET_DIGITAL1] & CELL_PAD_CTRL_L3)
				&& (pdata.button[CELL_PAD_BTN_OFFSET_DIGITAL2] & CELL_PAD_CTRL_L2) // fail-safe mode
				&& (pdata.button[CELL_PAD_BTN_OFFSET_DIGITAL2] & CELL_PAD_CTRL_R2) // SELECT+L3+L2+R2
				)
			{
			...
			if(!(sm_config->combo & SHOW_TEMP) && (pdata.button[CELL_PAD_BTN_OFFSET_DIGITAL1] & CELL_PAD_CTRL_R3) ) // SELECT+R3 show temperatures
			{
			...
			if(sm_config->fanc && !(sm_config->combo & MANUALFAN) && (pdata.button[CELL_PAD_BTN_OFFSET_DIGITAL1] & CELL_PAD_CTRL_UP) ) // SELECT+UP increase TEMP/FAN
			{
			...
			if(sm_config->fanc && !(sm_config->combo & MANUALFAN) && (pdata.button[CELL_PAD_BTN_OFFSET_DIGITAL1] & CELL_PAD_CTRL_DOWN) ) // SELECT+DOWN decrease TEMP/FAN
			{
				if(max_temp) //auto mode
				{
			...
			if(!(sm_config->combo & PREV_GAME) && (pdata.button[CELL_PAD_BTN_OFFSET_DIGITAL2] & CELL_PAD_CTRL_L1) ) // SELECT+L1 (previous title)
			{
				led(GREEN, BLINK);
			...
			if(!(sm_config->combo & NEXT_GAME) && (pdata.button[CELL_PAD_BTN_OFFSET_DIGITAL2] & CELL_PAD_CTRL_R1) ) // SELECT+R1 (next title)
			{
				led(GREEN, BLINK);
		...
		if ((pdata.button[CELL_PAD_BTN_OFFSET_DIGITAL1] & CELL_PAD_CTRL_L3) && (pdata.button[CELL_PAD_BTN_OFFSET_DIGITAL2] & CELL_PAD_CTRL_R2))
		{
			if(!(sm_config->combo & SHUT_DOWN) && (pdata.button[CELL_PAD_BTN_OFFSET_DIGITAL2] & CELL_PAD_CTRL_CROSS) ) // L3+R2+X (shutdown)
			{
				// power off
				working=0;
				// {system_call_4(379,0x1100,0,0,0);}
			...
			else if(!(sm_config->combo & RESTARTPS) && (pdata.button[CELL_PAD_BTN_OFFSET_DIGITAL2] & CELL_PAD_CTRL_CIRCLE) ) // L3+R2+O (restart)
			{
				// reboot
				working=0;
				// {system_call_4(379,0x1200,0,0,0);}
			...
			else if(!(sm_config->combo & UNLOAD_WM) && (pdata.button[CELL_PAD_BTN_OFFSET_DIGITAL1] & CELL_PAD_CTRL_R3)) // L3+R3+R2 (quit sMAN)
			{
				restore_fan(0);
				show_msg((char*)STR_WMUNL);
				goto leave;
			}
			else if((pdata.button[CELL_PAD_BTN_OFFSET_DIGITAL2] & CELL_PAD_CTRL_TRIANGLE) ) // L3+R2+/\ (enable/disable fancontrol)
			{
...
#define RES_VER		(9)					// resource files version
	...
	if(buf)
	{
		for(int n=0; n<res_files; n++)
		{
			...
			if(sm_config->resv==RES_VER && file_exists(dest)) continue;
...
int sm_stop(void){
	...
	sys_ppu_thread_create(&t, sm_stop_thread, 0, 0, 0x1000, SYS_PPU_THREAD_CREATE_JOINABLE, THREAD_STOP);
