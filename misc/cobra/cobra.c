...
{
	system_call_2(11, SYSCALL8_OPCODE_GET_VERSION, (uint64_t)(uint32_t)version);
	return (int)p1;
}

static int sys_get_version2(uint16_t *version)
{
	system_call_2(11, SYSCALL8_OPCODE_GET_VERSION2, (uint64_t)(uint32_t)version);
	return (int)p1;
}
*/
...
static inline int sys_read_cobra_config(CobraConfig *cfg)
{
	...
	system_call_2(11, SYSCALL8_OPCODE_READ_COBRA_CONFIG, (uint64_t)(uint32_t)cfg);
	return (int)p1;
}

static inline int sys_write_cobra_config(CobraConfig *cfg)
{
	system_call_2(11, SYSCALL8_OPCODE_WRITE_COBRA_CONFIG, (uint64_t)(uint32_t)cfg);
	return (int)p1;
}
...
{
	system_call_1(11, SYSCALL8_OPCODE_GET_ACCESS);
	return (int)p1;
}

static inline int sys_permissions_remove_access(void)
{
	system_call_1(11, SYSCALL8_OPCODE_REMOVE_ACCESS);
	return (int)p1;
}
/*
static inline int cobra_usb_command(uint8_t command, int requestType, uint32_t addr, void *buf, uint16_t size)
{
	system_call_6(11, SYSCALL8_OPCODE_COBRA_USB_COMMAND, command, requestType, addr, (uint64_t)(uint32_t)buf, size);
	return (int)p1;
}
...
{
	system_call_5(11, SYSCALL8_OPCODE_LOAD_VSH_PLUGIN, slot, (uint64_t)(uint32_t)path, (uint64_t)(uint32_t)arg, arg_size);
	return (int)p1;
}

int cobra_unload_vsh_plugin(unsigned int slot)
{
	system_call_2(11, SYSCALL8_OPCODE_UNLOAD_VSH_PLUGIN, slot);
	return (int)p1;
}
...
int cobra_panic(int mode) {
	system_call_2(11, SYSCALL8_OPCODE_PANIC, mode);
	return (int)p1;
}

int cobra_load_game_plugin(unsigned int slot, char *path, void *arg, uint32_t arg_size)
{
	system_call_5(11, SYSCALL8_OPCODE_LOAD_GAME_PLUGIN, slot, (uint64_t)(uint32_t)path, (uint64_t)(uint32_t)arg, arg_size);
	return (int)p1;
}

int cobra_create_process_thread(sys_pid_t pid, sys_ppu_thread_t *thread, void *entry, uint64_t arg, int prio, size_t stacksize, char *threadname)
{
	system_call_8(11, SYSCALL8_OPCODE_PROC_CREATE_THREAD, pid, (uint64_t)(uint32_t) thread, (uint64_t)(uint32_t) entry, arg, prio, stacksize, (uint64_t)(uint32_t) threadname);
	return (int)p1;
}
